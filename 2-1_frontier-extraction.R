...
harmonize <- function(Level, lower.level.df, higher.level.df){
  ...
  higher.level.df %<>%
    ...
    mutate(parent_ghecause = as.character(parent_ghecause))

  lower.level.df %<>% mutate(parent_ghecause = as.character(parent_ghecause))
...
for(i in levels){
  ...
  check <- frontier_harmonized %>%
    mutate(mece = getCauseInfo(return = i)) %>%
    filter(mece) %>%
    group_by(year, age) %>%
    dplyr::summarize(lower_summed = sum(frontier), .groups = "drop") %>%
