...
temp1 <- read.csv("data/input/chang_country.csv", as.is = TRUE) %>%
  mutate(year = floor(year),
         c.e_rho = (1- (1+rho)^(-ex-1)) * ((1+rho)/rho)) %>%
  dplyr::select(iso3, year, age, sex, c.e = ex, c.e_rho)
c.e40_rho <- temp1 %>%
  filter(age == 40) %>%
  dplyr::select(iso3, year, sex, c.e40_rho = c.e_rho)
e <- alpha %>%
  left_join(temp1, by = c("iso3", "year", "age", "sex")) %>%
  left_join(c.e40_rho, by = c("iso3", "year", "sex"))
...
temp1 <- read.csv("data/input/chang_frontier.csv", as.is = TRUE) %>%
  mutate(year = floor(year),
         f.e_rho = (1- (1+rho)^(-ex-1)) * ((1+rho)/rho)) %>%
...
check <- containsNA(VSL)
country_info$country[country_info$iso3 %in% unique(check$iso3)]

VSL %<>%
  filter(iso3 %notin% unique(check$iso3))
containsNA(VSL)
