...
### 310 - COUNTRY PROJECTION

# This script takes the harmonized country mortality rates for 2000-2019
...
# from the regression, in which case an average is used. This method is
# the same method used in the frontier projections.

# Returns:
# • output_data/country_projected.Rda (R dataset)
# • output_data/country_projection_info.Rda (R dataset)
...
tidy(); sourceEnv()
scriptName <- "310-country_projection"

# Loading data
sarahLoad(c("country_info", "ghe_recoded"))
...
country_projected <- full_join(temp3 %>% dplyr::select(-c(concern, age)),
                               Output, by = c("group", "iso3", "year", "age2", "age.f", "sex", "ghecause", "causename")) %>%
...
sarahSave(c("country_projected", "country_projection_info"))
