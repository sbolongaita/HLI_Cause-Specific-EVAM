...
# This script recodes the GHE causes of death so that causes classify as
# a mutually exclusive and collectively exhaustive set of causes that are
# of relevance for this analysis. It then creates a cause hierarchy dataframe
# and cause recode map dataframe for reference.

# Returns:
# • output_data/ghe_recode.R (R dataset)
# • output_data/cause_hierarchy.R (R dataset)
# • output_data/cause_recode_map.R (R dataset)
...
# Script name
scriptName <- "130-ghe_recode.R"

# Loading data
sarahLoad("ghe", folder = "data/processed")
source("project/utility/vectors.R")
...
# * 2.1 Level 1: Communicable, maternal, perinatal and nutritional co. ----

# Removing level 2:4 causes that are under communicable, maternal, perinatal
# and nutritional cond. causes
temp2 <- temp1 %>%
  filter(ghecause2 %notin% 20:590)


# * 2.2 Level 1: Noncommunicable diseases ---------------------------------
temp3 <- temp2 %>%
  # Removing level 3 and 4 causes that are under level 2 causes we're not interested in
  filter(ghecause2 %notin% c(811:814, 950:1010, 1030:1090, 1270:1320, 1331:1339, 1350:1390, 1410:1460, 1480:1502)) %>%
...
temp2 <- unique(temp1) %>%
  mutate(level = case_when(ghecause == 0 ~ 0,
                           ...
                           ghecause %in% c(610, 800, 820, 1100, 1170, 1210, 600600, 1520, 1600) ~ 2,
                           TRUE ~ 3),
         parent_ghecause = case_when(ghecause %in% c(10, 600, 1510) ~ 0,
                                     ghecause %in% c(610, 800, 820, 1100, 1170, 1210, 600600) ~ 600,
                                     ghecause %in% c(1130, 1140, 11001100) ~ 1100,
                                     ghecause %in% c(1230, 12101210) ~ 1210,
                                     ghecause %in% c(620, 630, 640, 660, 680, 700, 710, 610610) ~ 610,
         ...
         parent_causename = case_when(ghecause %in% c(10, 600, 1510) ~ "All causes",
                                      ghecause %in% c(610, 800, 820, 1100, 1170, 1210, 600600) ~ "Noncommunicable diseases",
                                      ghecause %in% c(1130, 1140, 11001100) ~ "Cardiovascular diseases",
                                      ghecause %in% c(1230, 12101210) ~ "Digestive diseases",
                                      ghecause %in% c(620, 630, 640, 660, 680, 700, 710, 610610) ~ "Malignant neoplasms",
