...
# This script creates tables for the appendix.
...
sourceEnv(); tidy()
scriptName <- "630-appendix_tables"
...
# Loading data
sarahLoad("cause_recode_map")

cause_recode_map$prefix <- getCauseInfo(cause_recode_map$recoded_ghecause, return = "prefix")
cause_recode_map$level <- getCauseInfo(cause_recode_map$recoded_ghecause, return = "level")
...
# Loading data
sarahLoad("frontier_projection_info")
...
temp1 <- frontier_projection_info %>%
  ...
  mutate(causename = case_when(sex == 1 ~ paste0(causename, " (males)"),
                               ...
                               TRUE ~ causename))
temp1$prefix = getCauseInfo(temp1$ghecause, return = "prefix")
temp1 %<>%
