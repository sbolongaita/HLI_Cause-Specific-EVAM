...
sourceEnv(); tidy()
scriptName <- "520-region_calculations"

# Loading data
sarahLoad(c("country_info", "frontier_scaled", "population", "region"))
...
region %<>%
  mutate(sex_match = ifelse(causename %in% sex.specific.causename, sex, NA)) %>%
...
rho = 0.03
temp1 <- read.csv("02-input_data/Chang_frontier-life-tables.csv", as.is = TRUE) %>%
...
for(i in seq_along(mece)){
  level <- names(mece[i])
  out <- p1
  out$mece <- getCauseInfo(out$ghecause, return = mece[i])
  out %<>%
    filter(mece) %>%
...
Y <- read.csv("02-input_data/WDI_GNI-per-capita.csv", as.is = TRUE, na.strings = c("", "..")) %>%
  filter(nchar(Time) <= 4) %>%
  dplyr::select(year = Time, iso3 = Country.Code,
                intl_2017 = GNI.per.capita..PPP..constant.2017.international.....NY.GNP.PCAP.PP.KD.,
...
region_pop <- left_join(country_info %>% filter(analysis_eligible) %>% dplyr::select(iso3, region),
                        population %>% filter(year == 2020) %>% dplyr::select(iso3 = iso3.region, pop = total_pop) %>% unique(),
                        by = "iso3") %>%
