...
sarahLoad(c("cause_hierarchy", "frontier_comparison"), folder = "data/processed")

figure_name <- "FigA1.pdf"

temp1 <- frontier_comparison %>%
...
for(i in unique(ggdata$causename)){
  ...
  figure <- ggplot(ggdata2) +
    ...
    scale_x_continuous("", breaks = c(seq(2000, 2019, 5)), labels = c("2000", "'05", "'10", "'15")) +
...
saveGGplot(grobs, figure_name, folder = "output/figures", width = 10, height = 7, multipage = TRUE)


figure_name <- "FigA1_alt.pdf"

ggdata <- temp1 %>%
  filter(year == 2019, is.na(sex) | sex == 2, age >= 30, level < 3,
         causename %notin% c("All causes", "Mental and substance use disorders",
                             "Intentional injuries")) %>%
  pivot_longer(cols = c("min", "p10"), names_to = "definition", values_to = "frontier") %>%
  mutate(definition = ifelse(definition == "min", "Minimum", "10th percentile"),
         causename = wrapper(causename, 28),
         zero = frontier < 0.1) %>%
  mutate(frontier = ifelse(frontier < 0.1, 0, frontier),
         causename = factor(causename, levels = wrapper(causename.levels, 28)))

colors <- colorFunct(2, names = c("Minimum", "10th percentile"))
shapes <- c(16, NA)

figure <- ggplot(ggdata) +
  facet_wrap(~ causename, ncol = 5) +
  geom_path(aes(x = age, y = frontier, group = definition, color = definition), linetype = "dotted") +
  ...
  geom_point(aes(x = age, y = frontier, group = definition, color = definition,
                 shape = zero), fill = "white") +
  scale_x_continuous("Age") +
  scale_y_continuous("Frontier mortality rate (per 100K)", trans = "log",
                     breaks = log.breaks, labels = log.labels, limits = c(0.1, NA)) +
  scale_color_manual("Frontier definition", values = colors) +
...
saveGGplot(figure, figure_name, folder = "output/figures", width = 12, height = 8)
